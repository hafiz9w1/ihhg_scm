...
class ScmEntryAddPackage(models.TransientModel):
    ...
    def _compute_package_already_added(self):
        for rec in self:
            rec.package_already_added_ids = (
                rec.scm_id.package_line_ids.package_id.ids)
    ...
    def action_confirm(self):
        ...
        for p in self.package_ids:
            if p not in self.scm_id.allocated_package_ids:
                ...
                package_line = self.scm_id.package_line_ids.filtered(
                    lambda r: r.package_id == p)

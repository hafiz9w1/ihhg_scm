...
class Package (models.Model):
    ...
    item_total = fields.Integer(
        compute='_compute_item_total',
        string='Total Item (per Package)',
    )
    delivery_address_ids = fields.Many2many(comodel_name='res.partner')
    secondary_address_ids = fields.One2many(
        comodel_name='ihh.package.secondary.address',
        inverse_name='package_id',
    )

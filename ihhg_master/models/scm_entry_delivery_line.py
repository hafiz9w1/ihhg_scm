...
class SelectionCriterium(models.Model):
    ...
    scm_id = fields.Many2one(comodel_name="scm.entry", string='SCM Reference', readonly=True, ondelete='cascade', index=True)
    scm_entry_package_line_id = fields.Many2one(comodel_name="scm.entry.package.line", readonly=True)
    # scm_entry_item_line_id = fields.Many2one(comodel_name="scm.entry.item.line", inverse_name="scm_package_line_id", readonly=True)
    scm_entry_item_line_ids = fields.One2many(string='Items', related='scm_entry_package_line_id.scm_entry_item_line_ids')

    delivery_address_id = fields.Many2one(comodel_name="res.partner", readonly=True)
    quantity = fields.Integer(readonly=True)
    package_id = fields.Many2one(string='Package', related='scm_entry_package_line_id.package_id')
    ...
    channel_id = fields.Many2one(string='Channel', related='scm_entry_package_line_id.channel_id')
    package_name = fields.Char(string='Package Name', related="scm_entry_package_line_id.package_id.long_name")

    # extra = fields.Text(string='Extra info...', related='scm_entry_item_line_ids.extra')
    ...
    manufacturing_company_name = fields.Text(string='Manufacturing Company Name', related='scm_entry_package_line_id.manufacturing_company_name')
    manufacturer_location = fields.Many2one(string='Manufacturer location', related='scm_entry_package_line_id.manufacturer_location')
    package_size = fields.Text(string='Package Size', related='scm_entry_package_line_id.package_size')
    package_weight = fields.Text(string='Package Weight', related='scm_entry_package_line_id.package_weight')
